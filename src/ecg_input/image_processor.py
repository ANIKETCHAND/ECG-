...
def process_ecg_image(
    ...
    if isinstance(file_or_path, np.ndarray):
        ...
        file_or_path.seek(0)
        file_bytes = np.asarray(bytearray(file_or_path.read()), dtype=np.uint8)
...
def _detect_ecg_grid(bgr_img: np.ndarray, gray: np.ndarray) -> bool:
    ...
    # 2. Fourier / morphological check for regular grid structure
...
def _detect_waveform_curves(gray: np.ndarray) -> bool:
    ...
    # Typical ECG traces have continuous connected edges with moderate density (2% to 25%)
